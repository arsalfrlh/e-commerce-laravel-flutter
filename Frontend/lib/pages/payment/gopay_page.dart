...
class GopayPage extends StatefulWidget {
  GopayPage({required this.payment, required this.beli});
  Payment payment;
  Beli beli;
...
class _GopayPageState extends State<GopayPage> {
  ...
  void initState() {
    ...
    totalController = TextEditingController(text: widget.beli.total.toString());
  }
      ...
      if(response['success'] == true){
        if(response['data']['transaction_status'] == 'settlement'){
          ...
          AwesomeDialog(
            ...
            desc: 'Silahkan lakukan pembayaran anda dengan QRIS',
            btnOkOnPress: (){},
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView(
        children: [
          ProductImages(payemnt: widget.payment),
          TopRoundedContainer(
            color: Colors.white,
            child: Column(
              children: [
                ...
                TopRoundedContainer(
                  ...
                  child: Column(
                    children: [
                      ColorDots(jumlah: jumlahController, total: totalController,),
                    ],
                  ),
                ),
              ],
...
class ProductImages extends StatefulWidget {
  const ProductImages({
    Key? key,
    required this.payemnt,
  }) : super(key: key);

  final Payment payemnt;
        ...
        SizedBox(
          ...
          child: AspectRatio(
            ...
            child: GestureDetector(
              onTap: () {
                final url = widget.payemnt.qrCode!; //url saat di klik
                Clipboard.setData(ClipboardData(text: url)); //akan di copy ke Clipboard saat di klik| yg berisi url tdi
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text('URL disalin ke clipboard')), //menampilkan pesan saat berhasil meng copy url
                );
              },
              child: CachedNetworkImage(
                imageUrl: widget.payemnt.qrCode!,
                fit: BoxFit.cover,
    ...
    return Column(
      ...
      children: [
        ...
        Padding(
          ...
          child: Text(
            "Anda memesan ${beli.namaBarang} dengan jumlah pesanan ${beli.jumlah}, Silahkan untuk melakukan pembayaran Anda dengan QRIS sebesar ${beli.total} sebelum ${payment.expired}",
            maxLines: 3,
          ),
        ),
        Padding(
...
class ColorDots extends StatelessWidget {
  ...
  final TextEditingController jumlah, total;
