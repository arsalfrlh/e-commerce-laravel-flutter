...
class BankPage extends StatefulWidget {
  BankPage({required this.payment, required this.beli});
  Payment payment; //model payment
  Beli beli; //model beli
...
class _BankPageState extends State<BankPage> {
  ...
  void initState() {
    super.initState();
    nmBarangController = TextEditingController(text: widget.beli.namaBarang);
    jumlahController = TextEditingController(text: widget.beli.jumlah.toString());
    totalController = TextEditingController(text: widget.beli.total.toString()); //model beli
    paymentCodeController = TextEditingController(text: widget.payment.paymentCode); //model payment
    expireController = TextEditingController(text: formated.format(widget.payment.expired));
    if(widget.payment.bank == 'bca'){
      bankController = TextEditingController(text: "Bank Central Asia");
    }else if(widget.payment.bank == 'bri'){
      bankController = TextEditingController(text: "Bank Rakyat Indonesia");
    }else if(widget.payment.bank == 'bni'){
      bankController = TextEditingController(text: "Bank Negara Indonesia");
    }else if(widget.payment.bank == 'cimb'){
      bankController = TextEditingController(text: "CIMB Niaga");
    }else{
      bankController = TextEditingController(text: "Error");
    }
  }

  void _cekStatus(BuildContext context)async{
    if(widget.payment.transaksiID.isNotEmpty){
      showDialog(
        context: context, 
        barrierDismissible: false,
        builder: (context) => const Center(child: CircularProgressIndicator(),));

      final response = await apiService.cekStatus(widget.payment.transaksiID);
      ...
      if(response['success'] == true){
        if(response['data']['transaction_status'] == 'settlement'){
          ...
          Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => ExpirePage()));
          ...
          AwesomeDialog(
            ...
            desc: 'Silahkan lakukan pembayaran anda dengan ${bankController.text}',
            btnOkOnPress: (){},
...
class SignUpForm extends StatelessWidget {
  SignUpForm({required this.nmBarang, required this.jumlah, required this.total, required this.bank, required this.paymentCode, required this.expire, required this.status});
  final TextEditingController nmBarang, jumlah, total, bank, paymentCode, expire;
